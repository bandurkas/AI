...
async function connectToWhatsApp() {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    if (!greetedUsers.has(sender)) {
      ...
      return; // ! Важно: не продолжаем обработку — только приветствие!
    }

    // 2. Короткий ответ на вопросы о стоимости/цене
    const priceKeywords = ["стоимость", "цена", "сколько стоит", "прайс", "price", "цены", "возможности"];
    ...
    // 3. Краткий, сбалансированный ответ через OpenAI
